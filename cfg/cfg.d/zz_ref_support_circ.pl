...
$c->{plugins}{"Screen::REF_Support::Circ::New"}{params}{disable} = 0;
$c->{plugins}{"Screen::REF_Support::Circ::Edit"}{params}{disable} = 0;
...
package EPrints::DataObj::REF_Support_Circ;

@EPrints::DataObj::REF_Support_Circ::ISA = qw( EPrints::DataObj );

sub get_dataset_id { "ref_support_circ" }
...
sub get_control_url { $_[0]->{session}->config( "userhome" )."?screen=REF_Support::EditCirc&circid=".$_[0]->get_id }

=item $list = EPrints::DataObj::REF_Support_Circ::search_by_user( $session, $user )
...
$c->{datasets}->{ref_support_circ} = {
	class => "EPrints::DataObj::REF_Support_Circ",
	sqlname => "ref_support_circ",
	name => "ref_support_circ",
	columns => [qw( circid )],
	index => 1,
	import => 1,
	search => {                
...
$c->{fields}->{ref_support_circ} = [] if !defined $c->{fields}->{ref_support_circ};
unshift @{$c->{fields}->{ref_support_circ}}, (
