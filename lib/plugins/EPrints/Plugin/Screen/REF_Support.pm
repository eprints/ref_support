...
sub can_be_viewed
{
	...
	return 0 unless( $self->{session}->current_user->exists_and_set( 'ref_support_uoa' ) || $self->{session}->current_user->exists_and_set( 'ref_support_uoa_role' ) );
...
sub can_select
{
	...
	return 0 if
		...
		!($self->{processor}->{role}->exists_and_set( 'ref_support_uoa' ) );	#|| $self->{processor}->{role}->exists_and_set( 'ref_uoa_role' ) );
...
sub render_warnings
{
	...
	if( !$self->{processor}->{role}->exists_and_set( 'ref_support_uoa' ) )
	{
...
sub current_benchmark
{
	...
	my $id = EPrints::Apache::AnApache::cookie(
		...
		"eprints_ref_support_benchmark"
	);
	
	if( $id )
	{
		$benchmark = $self->{session}->dataset( "ref_support_benchmark" )->dataobj( $id );
	}
	$benchmark = EPrints::DataObj::REF_Support_Benchmark->default( $self->{session} )
...
sub user_roles
{
	...
	{
		my $list = $self->{session}->call( "ref_support_roles_for_user",
				$self->{session},
				$user
			);
...
sub action_change_benchmark
{
	...
	my $benchmark = $self->{session}->dataset( "ref_support_benchmark" )->dataobj( $benchmarkid );
	return if !defined $benchmark;

	$self->set_cookie( "eprints_ref_support_benchmark", $benchmark->id );
}
...
sub action_reset_benchmark
{
	...
	$self->set_cookie( "eprints_ref_support_benchmark", "", 1 );
}
...
sub render_benchmarks
{
	...
	my $current_bm = EPrints::DataObj::REF_Support_Benchmark->default( $session );
	my $current_bmid = (defined $current_bm) ? $current_bm->get_id : -1;

	$session->dataset( "ref_support_benchmark" )->search->map(sub {
...
sub render_tools
{
	...
	my $tools = $self->render_action_list_short( "ref_support_tools" );
...
sub render_selections
{
	...
	my $selections = EPrints::DataObj::REF_Support_Selection::search_by_user( $repo, $processor->{role} );
	...
	foreach my $selection ($selections->get_records)
		...
		foreach my $otherid ( @{ EPrints::DataObj::REF_Support_Selection::who_selected( $repo, $eprintid ) || [] } )
		{
			if( $otherid == $roleid )
			{
			#	$already_selected = 1;
