package EPrints::Plugin::Screen::REF_Support::Report;
...
sub can_be_viewed
{
	...
	my %uoa_ids = map { $_->id => undef } @{ $uoas || [] };
        foreach my $uoa_role ( @{$user->value( 'ref_support_uoa_role' )||[]} )
        {
...
sub users_by_selection
{
	...
	foreach my $uoa (@uoas)
	{
        ...
        $list = $list->reorder( "ref_support_uoa/name" );
...
sub current_uoas
{
	...
	# selected UoA's
        ...
        my %allowed_uoas = map { $_ => undef } @{$self->{session}->current_user->get_value( 'ref_support_uoa_role' ) || [] };
...
EOJ
	...
	# looking at legacy data ? i.e. former benchmarks? This is slightly more complex to deal with
        my $benchmark = $self->{processor}->{benchmark};
        my $current_benchmark = EPrints::DataObj::REF_Support_Benchmark->default( $self->{session} );
	...
	$users->map( sub {
		...
		my $user_uoa = $user->value( 'ref_support_uoa' );
