package EPrints::Plugin::Screen::REF_Support::Benchmark::New;
...
sub new
{
	...
	my $actions_appear = [
		{
			place => "ref_support_benchmark_create",
			action => "create",
			position => 200,
		},
		{
			place => "ref_support_overview_benchmark",
			action => "create",
		},
		{
			place => "ref_support_benchmark_create",
	...
	if( defined $self->{session} 
		...
		&& defined $self->{session}->{datasets}->{ref_support_benchmark}
		&& defined EPrints::DataObj::REF_Support_Benchmark->default( $self->{session} ) )
	{
		push @$actions_appear,	{
				place => "ref_support_benchmark_create",
				action => "copy",
...
{ 
	...
	return $self->allow( 'ref_support_benchmark/create_new' ) if( defined $ds && ($ds->base_id eq 'ref_support_benchmark' || $ds->base_id eq 'ref_support_selection' ));
	return 0;
}
...
sub properties_from
{
	...
	my $datasetid = "ref_support_benchmark";
...
sub action_copy
{
	...
	$self->{processor}->{screenid} = 'REF_Support::Benchmark::Copy';
...
sub action_create
{
	...
	$self->{processor}->{screenid} = "REF_Support::Benchmark::Edit";
}
...
sub render
{
	...
	$chunk->appendChild( $self->render_action_list_bar( "ref_support_benchmark_create" ) );
