package EPrints::Plugin::Screen::REF_Support::Benchmark::Copy;
...
sub action_copy
{
	...
	$list->map( sub {
		...
		my @ref = @{ $object->get_value( 'ref_support' ) || [] };
	...
	my $url = $self->{session}->get_url."?screen=Workflow::Edit&dataset=ref_support_benchmark&dataobj=".$self->{processor}->{dataobj}->get_id;
...
sub render
{
	...
	my $bm = EPrints::DataObj::REF_Support_Benchmark->default( $session );
	unless( defined $bm )
	{
