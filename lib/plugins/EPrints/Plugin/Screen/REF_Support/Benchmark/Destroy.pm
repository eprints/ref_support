package EPrints::Plugin::Screen::REF_Support::Benchmark::Destroy;
...
sub can_be_viewed
{
	...
	return $self->allow( 'ref_support_benchmark/delete' );
}
...
sub properties_from
{
	...
	# sf2 - need to over-ride the {processor}->{benchmark} (set by default to the current BM). Here it needs to be set to $session->param( '
        $self->{processor}->{benchmark} = $session->dataset( 'ref_support_benchmark' )->dataobj( $session->param( 'dataobj' ) ); 
}
...
sub action_cancel
{
	...
	$self->{processor}->{dataset} = $self->{session}->dataset( 'ref_support_benchmark' ) unless( defined $self->{processor}->{dataset} );
}
