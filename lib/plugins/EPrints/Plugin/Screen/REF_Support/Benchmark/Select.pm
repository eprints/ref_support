package EPrints::Plugin::Screen::REF_Support::Benchmark::Select;
...
{ 
	...
	return $self->allow( 'ref_support_benchmark/select_current' ) if( defined $ds && ($ds->base_id eq 'ref_support_benchmark' || $ds->base_id eq 'ref_support_selection' ));
	return 0;
}
...
sub action_select_current
{
	...
	EPrints::DataObj::REF_Support_Benchmark::select_as_default( $self->{session}, $selected );
