package EPrints::Plugin::Screen::REF_Support::REF4::Listing;
...
sub can_be_viewed
{
	...
	return 1 if( defined $user && $user->exists_and_set( 'ref_support_uoa_role' ) );
...
sub render
{
	...
	my $ref_roles = $user->get_value( 'ref_support_uoa_role' );
...
sub render_result_row
{
	...
	my $session = $self->{session};
	my $benchmark = $self->{processor}->{benchmark};
	my $env_ds = $session->dataset("ref_support_env");
	my $searchexp = new EPrints::Search(
		...
		custom_order => "year",
                dataset=>$env_ds );
        $searchexp->add_field(
                $env_ds->get_field( "ref_support_benchmarkid" ),
                $benchmark_id );
 
        $searchexp->add_field(
                $env_ds->get_field( "ref_support_uoa" ),
                $uoa );
	...
	# create any ref_support_env entries that are required.
	foreach my $year (sort keys %$data)
	{
		if ($data->{$year}->{"id"} < 0)
		{
			my $new_item = $env_ds->create_dataobj();
			$new_item->set_value("ref_support_benchmarkid", [ $benchmark_id ] );
			$new_item->set_value("ref_support_uoa", [ $uoa ] );
	...
	foreach my $year (sort keys %$data)
	{
		...
		if ($data->{$year}->{"id"} >= 0)
		{
			...
			$edit_href->query_form(
				screen => "REF_Support::REF4::Edit",
				dataset => "ref_support_env",
				dataobj => $data->{$year}->{"id"},
				return_to => "REF_Support::REF4::Listing",
			);
