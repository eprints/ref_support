package EPrints::Plugin::Screen::REF_Support::Report::Listing;
...
sub render
{
	...
	foreach my $uoa ( sort {
		...
		my( $cmpa, $cmpb ) = ( $a, $b );
		if( $a =~ /^ref20[0-9]{2}_\w(\d+)b?/ )
		{
			$cmpa = $1;
		}
		if( $b =~ /^ref20[0-9]{2}_\w(\d+)b?/ )
		{
			$cmpb = $1;
		}
			return $cmpa <=> $cmpb; 
...
sub render_reports
{
	...
	my @reports = @{$self->{session}->config( 'ref_support', 'reports' ) || [] };

	my $select = $self->{session}->make_element( 'select', id => "report", name => "report" );
	my $n = 0;
	foreach my $reportid ( @reports )
	{
		my $plugin = $self->{session}->plugin( "Screen::REF_Support::Report::$reportid" ) or next;
...
{ 
	...
	my $users = $session->dataset( 'user' )->search( filters => [
                { meta_fields => [ "ref_support_uoa" ], value => $subject->id, },
        ]);
	...
	$benchmark->uoa_selections( $subject )->map(sub {
		...
		my $ok = 0;
		for(@{$selection->value( "ref_support" )})
		{
			$ok = 1, last
