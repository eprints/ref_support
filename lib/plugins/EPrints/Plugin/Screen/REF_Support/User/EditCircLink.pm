package EPrints::Plugin::Screen::REF_Support::User::EditCircLink;

# a link between user dataobj -> ref_support_circ dataobj
...
sub allow
{
        ...
        return $self->EPrints::Plugin::Screen::REF_Support::allow( @args );
}
...
sub from
{
	...
	my $circ = EPrints::DataObj::REF_Support_Circ->new_from_user( $self->{session}, $user->get_id, 1 );
	return unless( defined $circ );	# odd error
	
	my $redirect_url = $self->{session}->config( 'userhome' )."?screen=REF_Support::User::EditCirc&dataset=ref_support_circ&dataobj=".$circ->get_id;
