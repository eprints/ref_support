package EPrints::Plugin::Screen::REF_Support::User::EditCircLinkBack;

# the opposite of EditCircLink, a link between a ref_support_circ dataobj and a user object
...
sub can_be_viewed
{
        ...
        return 0 unless( defined $self->{processor}->{dataset} && $self->{processor}->{dataset}->id eq 'ref_support_circ' );
