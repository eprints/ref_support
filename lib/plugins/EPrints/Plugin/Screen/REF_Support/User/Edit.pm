package EPrints::Plugin::Screen::REF_Support::User::Edit;
...
sub properties_from
{
	...
	if( defined $session->param( 'userid' ) )
	{
		$self->{processor}->{dataobj} = $self->{processor}->{dataset}->dataobj( $session->param( 'userid' ) );
	}
...
sub from
{
	...
	$self->EPrints::Plugin::Screen::REF_Support::from;
}
