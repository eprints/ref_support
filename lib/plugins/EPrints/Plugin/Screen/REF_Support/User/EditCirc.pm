package EPrints::Plugin::Screen::REF_Support::User::EditCirc;
...
sub can_be_viewed
{
	...
	my $user = $self->{session}->current_user;
        ...
        {
                # but is he or she a champion for the user's uoa?
...
sub get_user_from_circ
{
	...
	return undef unless( defined $self->{processor}->{dataobj} );
        ...
        if( $self->{processor}->{dataobj}->get_dataset_id eq 'ref_support_circ' )
        {
...
sub properties_from
{
	...
	$self->{processor}->{role} = EPrints::Plugin::Screen::REF_Support::current_role( $self );
...
sub view_screen
{
	...
	return "REF_Support::User::EditCircLinkBack";
}
